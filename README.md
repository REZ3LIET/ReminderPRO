...
The [task reminder workflow](.github/workflows/task-reminders.yml) runs every day at 09:17 UTC and can also be started from the GitHub Actions tab with **Run workflow**. It reads [task data](data/task_reminders/tasks.json), runs Python to generate one report, and uses provider-specific Actions steps to post it to a Discord or Slack incoming webhook. Python does not access secrets or send notifications. The workflow never edits task data.
...
Omit `--today` to use the current UTC date. You can use `--output report.txt` to capture the message in a file. The workflow reads that file and builds the provider's JSON payload with `jq`, then sends it with `curl`. The Python script needs no third-party packages or a virtual environment.
